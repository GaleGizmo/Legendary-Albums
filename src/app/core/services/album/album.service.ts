...
export class AlbumService {
  ...
  public getAlbumAuthorByTitle(
    title: string
  ): Observable<{ album: AlbumI; author: AuthorI | undefined }> {
    this.isLoading = true;
    return this.apiAlbumService.getApiAlbumByTitle(title).pipe(
      map((apiAlbum: ApiAlbumI) => this.transformAlbum(apiAlbum)),
      switchMap((album) => {
        return this.apiAlbumService.getApiAuthors().pipe(
          map((apiAuthors: ApiAuthorI[]) =>
            apiAuthors.map((apiAuthor) => this.transformAuthor(apiAuthor))
          ),
          map((authors) => {
            const albumAuthor = authors.find(
              (author) => author.name === album.artist
            );
  ...
  public createAlbum(body: AlbumI): Observable<AlbumI> {
    return this.apiAlbumService.createApiAlbum(body).pipe(
      map((apialbum) => this.transformAlbum(apialbum)),
      catchError((error) => {
  ...
  public uploadImage(file: File): Observable<any> {
    ...
    return this.http.post(
      'https://api.cloudinary.com/v1_1/dwv0trjwd/image/upload',
      data
    );
  }
